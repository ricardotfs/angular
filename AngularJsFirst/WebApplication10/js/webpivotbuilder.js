...
// - jQueryUI sortable (used for select2 drag-and-drop, optional)
...
(function ($) {
	...
	function applySelect2($el, data) {
		...
		if (select2Obj.container) {
			// this is select2 3.x
			select2Obj.container.find("ul.select2-choices").sortable({
				containment: 'parent',
			...
			$el.on('change', function (e) {
				...
				$.each(uniqSelectedVals, function (i, id) {
					var elById = $.grep($elems, function (el) { return $(el).data("data").id == id; });
					$(elById).insertBefore($searchEl);
				});
			}).change();
			select2Obj.$container.find("ul").sortable({
				containment: 'parent',
	...
	PivotDataWebPivotBuilder.prototype.setPivotTableConfig = function (cube, pivotConfig) {
		...
		var dimsForPivot = $.grep(newCubeInfo.Dimensions, function (d) { return d.ReportType == null || d.ReportType != "Table"; });
	...
	DrillDownManager.prototype.handleDrillDown = function ($cellEl, cellIndex) {
		...
		for (var i = 0; i < cubeMetadata.Dimensions.length; i++) {
			...
			if ($.inArray(dim.Name, pvtBuilder.currentPivotData.Columns) < 0 &&
