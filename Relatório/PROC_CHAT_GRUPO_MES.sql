...
AS 
	BEGIN	
			...
			SELECT
				...
				AVG((CASE WHEN ISNULL(C.IDPROPRIETARIO,0) <> 0 THEN DATEDIFF(SECOND,C.DATAINICIO,ISNULL(DATEADD(HOUR,@HOUR,C.DATACONCLUSAO),DATEADD(HOUR,@HOUR,C.DATAINICIO))) ELSE 0 END)) TEMPO_MEDIO_ATENDIMENTO,
		  ...
		  ;WITH CHAT_TEMP AS (SELECT *,ROW_NUMBER() OVER(ORDER BY ANO DESC, MES_NUMERO DESC) AS ROWNUMBER
			...
			SELECT TOP (@QTD_PAGINA) 
				...
				CONVERT(VARCHAR, DATEADD(MS, ISNULL(TEMPO_MEDIO_ABONDONO,0) * 1000, 0), 108)[Tempo médio de perdido],
				OFFILINE [chat fora do horário de atendimento],
				CONVERT(VARCHAR, DATEADD(MS, ISNULL(TEMPO_MEDIO_ESPERA,0) * 1000, 0), 108) [Tempo Médio de espera para ser atendido],
				CONVERT(VARCHAR, DATEADD(MS, ISNULL(TEMPO_MEDIO_ATENDIMENTO,0) * 1000, 0), 108) [Tempo Médio de Atendimento],
